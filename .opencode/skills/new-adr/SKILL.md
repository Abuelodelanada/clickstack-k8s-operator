...
description: >-
  ...
  ADR, in docs/roadmap.md, in docs/stack-constraints.md, or in docs/BACKLOG.md.
...
| `docs/stack-constraints.md` | **Verified facts** about the workload — what the images and containers cannot do. | Is this true regardless of what we decide? |
...
- **Do not restate workload facts in an ADR.** Cite them:
  `[stack-constraints §4.2](../stack-constraints.md)`. When the images change,
  the fact is updated in one place and the ADR stays traceable to what was true
  when it was decided.

An observation is not an ADR. A verified fact is not an ADR. **A choice between
options is an ADR.**

This repo starts with zero ADRs. ADR-0001 is spoken for: it records the
topology decision (one charm, full stack in one pod, individual images) that
AGENTS.md carries provisionally in "Where we are" — writing it moves that
decision from the prose to the record it belongs in.
...
**Status:** Proposed
**Date:** <YYYY-MM-DD>
**Related:** [ADR-NNNN: <Title>](NNNN-slug.md), [Stack constraints reference](../stack-constraints.md)
...
Include the evidence — cite source files with line numbers, or
stack-constraints sections. If a fact is unverified, label it NOT VERIFIED.>
...
**Cite evidence, distinguish verified from assumed.** `docker-compose.yml:14`,
`docker/clickhouse/local/config.xml:30`, the `clickstack-stack` skill, a section
link, or a URL. Where something was not verified, write **NOT VERIFIED** — that
distinction is load-bearing in this repo, and an ADR that launders an assumption
into a fact is worse than one that admits the gap.
...
| Do not | Instead |
|---|---|
| Write an ADR for a decision with no alternative | It is a fact or an instruction — `stack-constraints.md` or the roadmap |
| Copy the `## Status` / `## Decision` heading form from generic ADR templates | Use the bold metadata block above; it is the house format |
...
| Restate the stack's behaviour inline | Cite `stack-constraints.md` by section |
