...
The circular-dependency escape does not apply because the `charm.py` → workload
layering is one-way by design. A function-level import to break a cycle is
evidence the layering broke; fix the layering.

`if TYPE_CHECKING:` blocks live at module top, so they are already compliant and
are the correct tool for type-only imports — this is how the workload modules
reference `ops.Container` without importing `ops` at runtime.

One caveat: **do not lint `lib/`.** If a Charmhub library is ever vendored there
(none is yet), it is third-party code we must not edit, and it may well contain
inline imports. The tox commands pass `src` and `tests` explicitly, so `lib/` is
already outside the lint scope — keep it that way rather than adding a
...
- **Naming carries intent.** PEP 8's overriding principle: names should reflect
  ...
  `ingestion_key` not `secret_string`.
...
- **Bare `except:` is forbidden.** Catch the narrowest exception that can
  actually occur. In this charm, `ops.pebble.ConnectionError`,
  `urllib.error.URLError`, or a workload module's own error class — not
  `Exception`.
...
def push_collector_config(self, config: Mapping[str, str]) -> None:
    """Render the collector's OTel config into the container and verify it.

    The rendered file is what the collector actually loads, so the write
    is followed by a read-back before the step is declared done.

    Args:
        config: The resolved config keys, already sorted at creation.

    Raises:
        OtelColError: The pushed bytes do not read back identical.
    """
```
...
- Document `Raises:` whenever the caller has to handle it. For this charm that
  is most of the workload modules — they own the read-backs.
...
`--python 3.14`, `uv` picks an interpreter that cannot parse this repo's
py314-only source — flaplint parses with the *running* interpreter's `ast`, so
its Python version is load-bearing. **Its published CI matrix was 3.10–3.13
(verified 2026-08 in the pi-hole repo); 3.14 support is NOT VERIFIED** — the
first run against real `src/` code is the verification, and a parser that fails
loudly is fine; one that skips modules silently is the risk.
...
rendering the collector's OTel config and the Pebble layers is exactly that
shape, do not treat a clean flaplint run as proof of ordering safety. Sort at
the source: `sorted()` when the collection is created, not when it is written.
...
than one maintainer. Its sinks include Pebble plans and rendered files, and it
was verified in the pi-hole repo on fixtures with no Kubernetes anywhere: true
positives on real churn, zero findings on clean workload-shaped code. It does
not assume a machine — but its 3.14-readiness is the open question above, so a
clean first run on this repo means less than it did there until proven.
