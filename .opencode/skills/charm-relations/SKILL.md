...
metadata:
  verified: "2026-09-15"
  source: >-
    Interface-ecosystem facts carried from the pi-hole repo's charm-relations
    (verified 2026-08); the candidate table is ClickStack-specific and
    decided-open. Re-check the interface library index before writing code.
---
...
Default answer to optionality for this charm: **`optional: true`**. The
ClickStack charm must reach `ActiveStatus` with zero relations — the stack is
a closed loop in one pod. Anything else needs an argument.
...
Our charm is a principal, so it declares no scope. But be aware that **every
...
precedence over the implicit `juju-info` one."* So a bare `juju integrate
clickstack other` may resolve to something other than the endpoint you meant.
**In integration tests and any deployment tooling, always name the endpoints
explicitly.**
...
- Do **not** create `lib/charms/clickstack/v0/*.py` for anything this repo owns.
...
Nothing is decided — zero relations exist today, and each row below is a
candidate that lands with an ADR. The one structural fact that shapes all of
them: **ClickStack is an observability backend**, so the provider/consumer
direction is the decision to make, not to copy.

| Interface | Role | Library | Status | Notes |
|---|---|---|---|---|
| `tracing` / OTLP | provides | `charmlibs.interfaces.otlp` (PyPI, 0.5.0 pre-1.0) or a Charmhub tracing lib — re-check the index | no badge / NOT VERIFIED | The stack's purpose: other charms ship OTLP to ClickStack's 4317/4318. The ingestion-key `authorization` header vs the interface's auth is the open question. Plausibly the first relation. |
| `metrics-endpoint` (`prometheus_scrape`) | provides | Charmhub `prometheus_scrape` lib (or PyPI successor) | re-check index | Collector self-metrics already exposed on 8888. |
| `logging` (`loki_push_api`) | provides | Charmhub `loki_push_api` lib (or PyPI successor) | re-check index | The containers' logs. |
| `grafana-dashboard` | provides | Charmhub `grafana_dashboard` lib (or PyPI successor) | re-check index | Dashboards for the stack. |
| `tls-certificates` | requires | `tls_certificates_interface.v4` | ✅ | Only if the UI/ingestion endpoints ever serve TLS. |
| peer | peers | none | — | Needed if multiple units ever agree on anything (key rotation broadcast, coordinated upgrades). |

`charmlibs.interfaces.otlp` communicates OTLP endpoints for workloads that
*already* speak OTLP — which is exactly this charm's ingestion surface, from
the other side. **NOT VERIFIED** whether its databag contract carries auth
material; check before designing the provider side around it.

## Declaring endpoints

```yaml
# Illustrative only — none of these exist yet, and each lands with an ADR.
provides:
  tracing:
    interface: tracing
    optional: true
...
```python
class IngestionProviderData(pydantic.BaseModel):
    """Data this charm publishes for OTLP senders."""

    endpoint: str
    port: int = 4318
    ...  # auth story: decided with the interface, not improvised


# write
relation.save(IngestionProviderData(endpoint=ingress), relation.data[self.app])

# read
data = relation.load(IngestionProviderData, relation.data[remote_app])
```

Only write a manual `dump`/`load` pair if a library forces a wire format that
`Relation.save` cannot produce. Note the ops docstring for `Relation.load`
...
```python
def _reconcile_ingestion_relations(self) -> None:
    """Publish the ingestion endpoint to every consumer. Safe with zero relations."""
    if not self.unit.is_leader():
        return
    data = IngestionProviderData(endpoint=self._ingress_address)
    for relation in self.model.relations["tracing"]:
...
For a single-relation endpoint, `self.model.get_relation("tracing")` returns
